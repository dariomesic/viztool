      ...
      <div class="modal-wrapper">
        <div class="modal-container" style="width:550px">
            <div style="overflow: hidden;text-overflow: ellipsis;white-space: nowrap;margin: 0 -5px;padding: 0 5px;font-size:15px">
                <h2 style="margin:15px;">Edit Condition Format</h2>
            </div>
              ...
              <div class="format_content">
                <table class="formats">
                  ...
                  <tr>
                    <td style="padding-bottom: 14px;padding-right: 18px;">
                      <div style="border-width: 0 0 1px 0;;border-color: rgba(0,0,0,.42);">
                        <select class="format_select" @change="header = $event.target.value" v-model="conditional.header">
                    ...
                    <td style="padding-bottom: 14px;padding-right: 18px;">
                      <div style="border-width: 0 0 1px 0;;border-color: rgba(0,0,0,.42);">
                        <select class="format_select" @change="sign = $event.target.value" v-model="conditional.sign">
                          <option>Sign</option>
                    ...
                    <td style="padding-bottom: 14px;padding-right: 18px;white-space: nowrap;">
                      <div v-if="conditional.sign === 'Between'" style="border-width: 0 0 1px 0;;border-color: rgba(0,0,0,.42);">
                        <div class="input-name" style="width:45px;margin-right: 10px;;margin-bottom: 7px;border-width: 0 0 1px 0;">
                            <input type="text" placeholder="0" style="margin-left:0px;width:120px" v-model="conditional.between[0]">
                        ...
                        <div class="input-name" style="width:45px;box-sizing: content-box;margin-right: 10px;padding-left: 0;;margin-bottom: 7px;border-width: 0 0 1px 0;">
                            <input type="text" placeholder="0" style="margin-left:0px;width:120px" v-model="conditional.between[1]">
                            <span class="underline-animation"></span>
                        </div>
                      </div>
                      <div v-else style="border-width: 0 0 1px 0;;border-color: rgba(0,0,0,.42);">
                        <div class="input-name">
                            <input type="text" placeholder="0" style="margin-left:0px;width:120px" v-model="conditional.value">
                            <span class="underline-animation"></span>
                        </div>
                      </div>
                    </td>
                  ...
                  <tr>
                    <td style="padding-bottom: 14px;padding-right: 18px;">
                      <div style="border-width: 0 0 1px 0;;border-color: rgba(0,0,0,.42);">
                        <select class="format_select" @change="format = $event.target.value" v-model="conditional.format">
                    ...
                    <td style="padding-bottom: 14px;padding-right: 18px;">
                      <div style="border-width: 0 0 1px 0;;border-color: rgba(0,0,0,.42);display:inline-flex">
                        <select class="format_select" @change="fontSize = $event.target.value" v-model="conditional.fontSize">
                          <option>9px</option>
                          <option>10px</option>
                          <option>11px</option>
                          <option>12px</option>
                    ...
                    <td style="padding-bottom: 14px;">
                      <div style="border-width: 0 0 1px 0;;border-color: rgba(0,0,0,.42);display: flex;">
                        ...
                        <palette 
                          ...
                          :selected="conditional.colorCell ? conditional.colorCell : 'rgb(255, 255, 255, 0.99)'" 
                        ...
                        <palette
                          ...
                          :selected="conditional.colorText ? conditional.colorText : 'rgb(0, 0, 0, 0.99)'" 
                  ...
                  <tr style="display:flex;margin-top:-10px">
                     <td>
                        <div class="checkbox" @click="conditional.bold = !conditional.bold">
                          <img class="checkbox-icon" :src="require(`@/assets/bold_icon.png`)" :class="{ 'hovered': conditional.bold}" style="width:25px;height:25px">
                        </div>
                     </td>
                     <td>
                        <div class="checkbox" @click="conditional.italic = !conditional.italic" style="margin-left:15px">
                          <img class="checkbox-icon" :src="require(`@/assets/italic_icon.png`)" :class="{ 'hovered': conditional.italic}" style="width:25px;height:25px;">
                        </div>
                     </td>
                     <td>
                        <div class="checkbox" @click="conditional.underline = !conditional.underline" style="margin-left:15px">
                          <img class="checkbox-icon" :src="require(`@/assets/underline_icon.png`)" :class="{ 'hovered': conditional.underline}" style="width:25px;height:25px;">
                        </div>
                     </td>          
                  </tr>
                </table>
...
export default {
  ...
  data(){
    return{
      ...
      conditional: this.data
    }
  },
  methods:{
    setColorText(value){
      this.conditional.colorText = value
    },
    setColorCell(value){
      this.conditional.colorCell = value
    },
    setData(){
      if(this.conditional.sign === 'Between'){
        this.conditional.value = ''
      }
      this.$emit('changeData', {data : this.conditional, index: this.id})
      this.$emit('close')
    }
  },
  watch:{
    data: {
        handler(val){
          this.conditional = JSON.parse(JSON.stringify(val));
        },
        deep: true
    },
  },
