...
ChartJS.register(Title, Tooltip, Legend, LineElement, LinearScale, PointElement, CategoryScale, Filler, ChartJSPluginDatalabels)
...
export default {
  ...
  methods: {
    ...
    onDrop (evt, list) {
        ...
        this.$nextTick(() => {
          if (this.$refs.line.chart.config.data.datasets[0]){
            let values_max = []
            let values_min = []
            this.$refs.line.chart.config.data.datasets.forEach((y) => {
