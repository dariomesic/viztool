...
export default {
  components : {Bar, ParaterizedSlot},
  props: {
    editMode: {
      type: Boolean,
      default: false
  ...
  computed: {
    ...
    chartOptions: function () {
      let options = {
          ...
          indexAxis: (this.Column.rotation === undefined || this.Column.rotation === false) ? 'x' : 'y',
  ...
  methods: {
    ...
    onDrop (evt, list) {
        ...
        this.$nextTick(() => {
          if (this.$refs.bar.chart.config.data.datasets[0]){
            let values_max = []
            let values_min = []
            this.$refs.bar.chart.config.data.datasets.forEach((y) => {
